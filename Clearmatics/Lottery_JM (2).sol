 ...
 * @notice This contract uses Chainlink VRF to generate random numbers for the winning combination.
 */
...
contract Lottery is VRFV2WrapperConsumerBase, ReentrancyGuard, Ownable {
    ...
    function computeWinningNumbers() external onlyOwner {
        ...
        for (uint256 i = 0; i < winningArr.length; i++) {
            winningNumbers.push(winningArr[i] % maxNumber);
        }
...
// @notice Computes the winning numbers for a lottery game.
// @dev Can only be called by the contract owner and only when block.number is greater than or equal to T. Winning numbers are generated pseudorandomly using the block hash, prevrandao, and the loop index. The winning numbers are then sorted in ascending order. This function can only be called once.
